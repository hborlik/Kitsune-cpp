...
struct bpf_object *load_bpf_object_file(const char *filename)
{
    ...
    if (obj == NULL) {
        fprintf(stderr, "ERR: opening and loading BPF-OBJ file(%s) (%d): %s\n",
    ...
    //  */
    // err = bpf_prog_load(prog_type, &prog_load_attr, &obj, &first_prog_fd);
    // if (err) {
...
struct bpf_object *load_bpf_and_xdp_attach(struct config *cfg)
{
    ...
    strncpy(cfg->progsec, bpf_program__name(bpf_prog), sizeof(cfg->progsec));
