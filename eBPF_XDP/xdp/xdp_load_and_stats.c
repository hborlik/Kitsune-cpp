			...
			struct stats_record *stats_prev)
{
	...
	double mbps; /* M bytes per sec */

	for (__u32 key = XDP_ABORTED; key < XDP_ACTION_MAX; key++)
	{
		char *fmt = 
		...
		printf(fmt, action, rec->total.rx_packets, pps, rec->total.rx_bytes / 1000, mbps, period);
	}
}
...
exit:
	...
	if (verbose) {
		printf(" - XDP prog detached on device:%s(ifindex:%d)\n",
		       cfg.ifname, cfg.ifindex);
	}
